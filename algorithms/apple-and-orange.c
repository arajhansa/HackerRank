...
void countApplesAndOranges(int s, int t, int a, int b, int apples_count, int* apples, int oranges_count, int* oranges) {
    ...
    int apples_onHouse = 0, oranges_onHouse = 0;

    for (int i=0; i<apples_count; i++) {
        apples[i] += a;
        if (apples[i] >= s && apples[i] <= t) apples_onHouse++;
    }    
    for (int i=0; i<oranges_count; i++) {
        oranges[i] += b;
        if (oranges[i] >= s && oranges[i] <= t) oranges_onHouse++;
    }

    printf("%d\n%d", apples_onHouse, oranges_onHouse);
